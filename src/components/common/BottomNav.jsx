      ...
      <div className="grid grid-cols-5 gap-1">
        {NAV_ITEMS.map((item) => (
          <NavLink
            ...
            to={item.href}
            className={({ isActive }) =>
              `flex min-h-[64px] flex-col items-center justify-center rounded-2xl px-1 text-[11px] font-medium transition-colors ${isActive ? 'bg-slate-800 text-white' : 'text-slate-400'
              }`
            }
          >
            {({ isActive }) => (
